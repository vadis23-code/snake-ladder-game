...
### Auth model
- Supabase passwordless email OTP authentication
- The browser requests a code with `signInWithOtp({ email, options: { shouldCreateUser: true } })`
- The browser verifies the six-digit code with `verifyOtp({ email, token, type: 'email' })`
- OTP values are memory-only: never generated by CourtCall, persisted, exported, or logged
- The Supabase Magic Link email template must contain `{{ .Token }}` for six-digit delivery
- Legacy PBKDF2 PIN verification remains available only for pre-existing device-local profiles; cloud PIN/password sign-in is retired
...
- `reset-pin` — legacy hash alias only; redirects to the Profile email-code screen with reason `legacy_pin_retired` (there is no `s-reset-pin` DOM screen)
...
`_PUBLIC_SCREENS` contains screens accessible without a cloud identity: `landing`, `profile`, `onboarding`, `privacy`, `terms`, `communities`, `world`, and `help-center`.
...
Passwordless authentication and local guest-profile screen. The root application still opens on the dedicated cinematic Landing screen.
...
**Email step** (`auth-email-form`):
- Email input: `id=inp-si-email`, `type=email`, `autocomplete=email`
- Explicit Privacy Policy / Terms consent
- "Send Code" calls `sendOtpCode()`
- The same request supports new and returning accounts without disclosing account existence

**Verification step** (`auth-verify-form`):
- One accessible input: `id=inp-otp-code`, `inputmode=numeric`, `autocomplete=one-time-code`, `maxlength=6`
- Paste and full keyboard input are supported
- "Verify" calls `verifyOtpCode()`
- "Change email" returns to the email step
- Resend is disabled during the 60-second cooldown and while a request is in flight
- `auth-status` is a polite live region and is referenced by the email/code inputs

**Returning user flow:**
- If profile exists (`K.profile` in localStorage), pre-fill email in sign-in tab
- A valid persisted Supabase session restores without requesting another OTP
- After verification, resolve `profiles.id = supaUser.id`; reuse the existing profile or create the same-ID row if absent
- Restore an intended private deep link or the Landing CTA intent; first-use accounts may enter the existing onboarding screen

**Guest flow:**
- "Continue as Guest" calls `guestLogin()`, sets `currentProfile.id = 'guest'`, and skips Supabase
- Optional local-profile fields remain available under "Customize Local Profile"
- Later account sign-in offers the existing explicit local-data sync decision; decline keeps guest data local

**Auth functions:**
- `supaSendOtp(email)` delegates to `CourtCallAuth.requestEmailOtp()`
- `supaVerifyOtp(email, token)` delegates to `CourtCallAuth.verifyEmailOtp()`
- `_finishAuthenticatedIdentity()` hydrates the CourtCall profile, restores routing, then starts background sync
- `restoreAuthenticatedSession()` uses `auth.getSession()` and never requests a new code
- `legacyLocalSignIn()` is isolated to stored `localOnly` profiles with an existing PBKDF2/legacy verifier
...
### 4.18 Legacy Reset PIN Compatibility
The password/PIN reset screen is retired. `#/reset-pin` safely redirects to the Profile email-code screen with route reason `legacy_pin_retired`. A valid older Supabase recovery link is accepted as an authenticated event and enters the normal profile-hydration path; CourtCall does not prompt for or update a cloud password.
...
```
Loaded from the pinned local browser bundle: `basketball-supa.js` (Supabase JS 2.106.2).

### Auth state
- `supaUser` — Supabase Auth user object or null
- `supaSession` — valid session with an access token and user ID, or null
- On init: `supa.auth.getSession()` restores a persisted session
...
- Auth email and OTP inputs have explicit labels and shared live status/error descriptions
...
- The single OTP input uses `inputmode="numeric"`, `autocomplete="one-time-code"`, `aria-invalid`, and full paste support
- Legacy local-PIN boxes retain `role="group"`, per-digit labels, keyboard behavior, and an accessible visibility toggle
...
### Profile
- **Switch Profile / Sign In:** ends the current session and opens the email-code flow
- **Local Guest Profile:** can be customized without creating a Supabase account
...
- Removes only this project's local Supabase session record and `K.profile`
- Preserves matches, tournaments, communities, settings, backups, and other non-auth device data
- Navigates to the public Landing screen
...
4. Initialize the OTP input and optional `pin-legacy` compatibility row
5. If profile exists:
   ...
   h. Else: showScreen('landing')
6. If no profile:
   a. If hash is to a public screen: _routeFromHash()
   b. Else: showScreen('landing')
7. `restoreAuthenticatedSession()` validates and hydrates any persisted Supabase session without bypassing root Landing
8. Apply saved settings (default win score chip)
9. Load builder state from 'cc_builder'
10. Wire ARIA tablist roving tabindex on `#tc-tabs`, `#cd-tabs`, `#history-filter-bar`, and `#tb-tabs-bar`
11. Version toast check
```
